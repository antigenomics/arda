...
Pinned to **arda 2.23.0** (`environment.yml`, the `container` tag, and the `Dockerfile`).
...
- **`-profile conda`** works out of the box (Nextflow builds the env from `environment.yml`) — once
  arda 2.23.0 is on PyPI; see the note at the top.
...
5. **Container override** (only for `-profile docker/singularity/<your-profile>`): add
   `withName: 'ARDA' { container = '<your-registry>/arda-mapper:2.23.0' }` to your deployment
