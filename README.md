...
this repo and regenerated by `python examples/regenerate.py`: one mRNA per locus; the two human
reads (of 7,341) that carry a **tandem D-D**; six VDJdb records covering every junction-repair
outcome, including one arda reports and refuses to rewrite; and a 1,035-read FASTQ that runs the
whole bulk RNA-seq pipeline in ~6 s. See [`benchmarks/RESULTS.md`](benchmarks/RESULTS.md) for
